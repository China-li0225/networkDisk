<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:userGroup:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="userGroupList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="岗位ID" align="center" prop="groupId" v-if="true"/>
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getUserGroup(userId).then(response => {
        this.loading = false;
        this.form = response.data;
        this.open = true;
        this.title = "修改用户与用户组关联";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用户与用户组关联编号为"' + userIds + '"的数据项？').then(() => {
        this.loading = true;
        return delUserGroup(userIds);
      }).then(() => {
        this.loading = false;
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/userGroup/export', {
        ...this.queryParams
